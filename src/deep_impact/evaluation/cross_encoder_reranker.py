    ...
    def rerank(self, qid: int, pbar):
        ...
        for i, pid in enumerate(top_k_pids):
            ...
            if len(batch_encoded_list) == self.batch_size or i == len(top_k_pids) - 1:
                input_ids = torch.tensor([x.ids for x in batch_encoded_list], dtype=torch.long).to(DEVICE)
                attention_mask = torch.tensor([x.attention_mask for x in batch_encoded_list], dtype=torch.long).to(
                    DEVICE)
                type_ids = torch.tensor([x.type_ids for x in batch_encoded_list], dtype=torch.long).to(DEVICE)
